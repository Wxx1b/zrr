jQuery.fn.extend({
    stk_version:function(){return "3.0.0";},
        ...
        if(data == undefined)
        {
            ...
            else if(cn == "classifiedselect" || cn=="letterselect")
            {
                ...
                if(typeof(data) == typeof([]))
                {
                    ...
                    {
                        $("#"+id).find("input[name!='classify'][value='"+data[i]+"']").prop("checked",true).change();
                    }
                }
            }
    ...
    stk_text:function(data) {
        ...
        var wrap = $("#" + id + "_wrap");
        if(wrap.length == 0){
            wrap = this.parents("div.stk_component_wrap:eq(0)");
        }
        var cn = $(wrap).attr("cn");
        if(data == undefined)
        {
    ...
    stk_emptyOptions:function() {
        ...
        if(cn == "removableselect")
        {
            ...
            $(obj).find("input[type='checkbox']").prop("checked",false).change();
        }
    ...
    stk_attr: function(attr,value) {
        ...
        if(value == undefined)
        {
            ...
            {
                return (attr == "selected" || attr == "checked") ? $(obj).prop(attr) : $(obj).attr(attr);
            }
        }
        else
        {
            ...
            {
                (attr == "selected" || attr == "checked") ? $(obj).prop(attr,value) : $(obj).attr(attr,value);
            }
            return $(this);
        }
    },
    stk_disabled: function(bool) {
        ...
        {
            var input = wrap.find("input").prop("disabled", bool);
            if(wrap.attr('ver') == 'V3') {
                input.iCheck('update');
            }
        }
        ...
        {
            $("#"+id+"_showValue").prop("disabled",bool);
            $("#"+id+"_multiVisibleDiv").find("input").prop("disabled",bool);
        }
        else if (cn == "doceditor")
        {
        ...
        {
            if (bool)
                $(obj).removeClass("stk-button").find("span").css("color", "gray");
            else
        ...
        {
            obj = $(obj).prop("disabled", bool);
            if(wrap.attr('ver') == 'V3') {
                if(obj[0].tagName == 'SELECT' && document.documentMode === 9){
                    obj.select2("enable", !bool);
                } else if(obj[0].tagName == 'INPUT' && (obj[0].type == 'radio' || obj[0].type == 'checkbox')) {
                    obj.iCheck('update');
                }
            }
        }
    ...
    stk_addOptions:function(para1, para2) {
        ...
        else if(cn == "multiselect")
        {
            ...
            obj = $("#"+id+"_multiSelDiv");
            var selectAll = $("#"+id+"_cb_all").prop("checked", false).change();
            var html = [];
            for ( var key in para1 )
            {
    ...
    stk_removeOptions:function(para1) {
        ...
        else if(cn == "classifiedselect")
        {
            for ( var i = 0; i < para1.length; i++ )
            {
               $(obj).find("input[name!='classify'][value='"+para1[i]+"']").prop("checked",false).change();
               $(obj).find("input[name!='classify'][value='"+para1[i]+"']").parent().parent().parent().find("input[name='classify']").prop("checked",false).change();
            }
        }

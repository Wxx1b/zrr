...
String.prototype.toDate = function() {
    ...
    {
        oldDate = this.substr(0,10);
        oldTime = this.substr(11,8);
    }
    else if(this.length == 16)
    {
        oldDate = this.substr(0,10);
        oldTime = this.substr(11,5) + ":00";
    }
...
StringUtils.hasChina = function( s )
{
    var patrn = /[\u4E00-\u9FA5]|[\uFE30-\uFFA0]/gi;
    return patrn.exec(s);
}
StringUtils.htmlEscape = function(text)
{
...
StringUtils.htmlUnescape = function(html)
{
    var g_oHtmlEncodeElement;
    g_oHtmlEncodeElement = g_oHtmlEncodeElement || document.createElement("div");
...
<!-- ################################################################## -->
var DwrUtils = {};
DwrUtils.getValue = function(ele, options) {
    function _byId() {
        var elems = [];
    ...
    function _isHTMLElement(ele, nodeName) {
        ...
        if (nodeName != null) {
            ...
            alert("dwr.util._isHTMLElement was passed test node name that is neither a string or array of strings");
            return false;
        }
        return true;
    }

    var _escapeHtml = true;

    function _shouldEscapeHtml(options) {
  ...
  var orig = ele;
  ele = _byId(ele); // Returns null if argument is a name, even on IE
  var nodes = null;
  if (ele == null) {
...
CnToPy.ConvertPinyinFirst = function(l1) {
    var l2 = l1.length;
    var I1 = "";
    var reg = new RegExp('[a-zA-Z0-9\- ]');
    for (var i=0;i<l2;i++) {
...
// ڶ
CnToPy.arraySearch = function(l1,l2){
    for (var name in CnToPy.PinYin){
        if (CnToPy.PinYin[name].indexOf(l1)!=-1) {
            return CnToPy.ucfirst(name); break;
        }
    }
    return false;
}

CnToPy.ucfirst = function(l1){
    if (l1.length>0) {

    ...
    $.each(columnConfig, function( i, n )
    {
        ...
        if(StringUtils.isNotBlank(n.pluginType))
        {
            ...
            for(var i = 0; i < plugins.length; i++)
            {
                ...
                {
                    var cb = $("<input type='checkbox' id='"+pluginParams[i]+"_all'>");
                    var sl = "tr[type!='group'][type!='groupTop'][fake!='1'] > th";
                    if(stk.table.isFixTable(id))
                    {
                        var hlbox = $("#"+n.tableId+"_HL").find(sl);
                        if(hlbox.length == 0)
                        {
                            $("#"+n.tableId+"_HT").find(sl).eq(i).empty().append(cb);
                        }
                        hlbox.eq(i).empty().append(cb);
                    } else {
                        $("#"+n.tableId).find(sl).eq(i).empty().append(cb);
                    }
                ...
                if(plugintypes[i]=='radiobox' && tableConfig.versionMode == 'V3')
                {
                    var radios = $("input[type=radio][name='" + pluginParams[i] + "']");
                    radios.iCheck({
                        radioClass: 'radio'
                    }).change(function(){
                        $(this).iCheck('update')
...
stk.table.adjustColWidth = function(id,pageInfo)
{
    ...
    $.each( $( table ).find( "th[plugin]" ), function ( i, n )
    {
        ...
        {
            width = $("#"+id+"_wrap").attr('ver') == 'V3' ? 45 : 20;
        }
        if(width)
        {
...
stk.table.exportExcel = function(id, isAll)
{
    ...
    SpiritTableAction.exportExcel(tableConfig, columnConfig,stk.table.getQueryForm(window["_"+id]), stk.table.getOrderBy(id), fileName,ids , {
        ...
        {
            var isIE = !+'\v1';
            if(isIE || !!window.XDomainRequest || window.navigator.msPointerEnabled){
                ...
                var form = $( "#" + id + "_tableExportForm" );
                if ( form.length == 0 )
                {
                    form = $( "<form action='" + __contextPath + "/webcontrol.action' method='post' id='" + id + "_tableExportForm'><input type='hidden' name='fileName'></form>" );
...
stk.table.adjustScroll = function(id)
{
    ...
    if(stk.table.isFixTable(id) && !!window.XDomainRequest && !window.navigator.msPointerEnabled){//is IE 9-
        if(!gr)
            gr = $('#'+id+'_grid');
...
stk.table.genPageInfo = function(config,pageInfo)
{
    ...
    if(pageSize != undefined && pageSize != null && pageSize > 1)
    {
        ...
        html.push(">");
        if(config.tableConfig.versionMode=="V3"){
            html.push("首页")
        }
        html.push("</a>");
...
stk.table.getCheckedData = function (config, name,allowCache)
{
    ...
    $.each(_tr, function(i, n)
    {
        ...
        if($(n).prop("checked") && _cache[objid] == undefined)
...
stk.alert = function(param1,param2,param3)
{
    var title;
    var buttons;
    if(param3 == undefined) param3 = {"确认":function(){$("#__alertDiv").dialog("close");return false;}};
    if($("#__alertDiv").length == 0)
    {
...
stk.checkBox.setValue = function( name,obj, sign)
{
    ...
    if(typeof(obj) == 'string')
        ...
        strArrays = obj;
    $("input[name='" + name + "']").each(function(j, h){
        h = $(h);
        var v = h.val();
        if(h.prop("checked")){
            if(strArrays.indexOf(v) == -1){
                h.prop("checked", false).change();
            }
        } else {
            if(strArrays.indexOf(v) > -1){
                h.prop("checked", true).change();
            }
        }
    });
};
...
stk.checkBox.buildSelectAll = function(id, targetName)
{
    ...
    var com = obj.parents('div.stk_component_wrap');
    if(com.attr('ver') == 'V3'){
        var opt = {
            checkboxClass: 'checkbox',
            radioClass: 'radio'
        };
        checkboxs.iCheck(opt).change(function(){
            $(this).iCheck('update')
        }).on('ifChanged', function(){
            $(this).change();
        }).on('ifClicked', function(){
        ...
        obj.iCheck(opt).change(function(){
            obj.iCheck('update');
        }).on('ifChanged', function(){
            obj.change();
        }).on('ifClick', function(){
    ...
    obj.change(function () {
        ...
        checkboxs.prop("checked", ck).change();
    });
...
stk.radioBox.setValue = function (name, value) {
    ...
    radios.filter(":checked").prop("checked", false).change();
    radios.filter("[value="+value+"]").prop("checked", true).change();
    var com = radios.parents('div.stk_component_wrap');
    if(com.attr('ver') == 'V3'){
        setTimeout(function(){
            radios.iCheck('update');
        },1);
    }
};
<!-- ################################################################## -->
...
stk.form.getParameters = function( id, options, sign )
{
    ...
    function putMap(n)
    {
        if(!stk.textField.isEmptyText(n))
        {
            ...
            else if($(n).attr("id")) map[$(n).attr("id")] = $(n).stk_val();
        }
    }

    $.each($("#" + id).find("input"), function(i, n)
    {
        var type = $(n).prop("type") || 'text';
        var value = $(n).val();
...
stk.classifiedSelector.init = function(id,result,options)
{
    ...
    if(options.multiple == "true")
    {
        ...
        {
            var p = $("#"+id)
            //var cb = p.find("div[name='content']" ).find("input[type='checkbox']" );
            /*$("#class_all_"+options.id ).bind("change",function(){
                cb.prop("checked",$(this).prop("checked")).change()
            });*/
            stk.checkBox.buildSelectAll("class_all"+id+"_"+options.id,"classify");
            p.find("input[type='checkbox'][nmame!='classify_all']" ).bind("change",function(){
                var div = p;
                div.find("input[type='checkbox'][name='classify_all']" ).prop("checked",div.find("input[type='checkbox'][name!='classify_all']" ).length == div.find("input[type='checkbox'][name!='classify_all']:checked" ).length).change()
            })
        }
    }
    else
...
{
    var multiple = obj.getAttribute("multiple");
    ...
    else
    {
        ...
        {
            var n = $(n);
            var label = n.parents('span:eq(0)').find('label[for=' + n[0].id + ']');
            _data[n.val()] = label.text();
        });
...
stk.letterSelector.init = function(id,result,options)
{
    ...
    var cont = $("#"+id).find("div[name='content']").html(html.join("\n"));
    if(options.multiple == "true")
    {
        var btng = $("#"+id).find("div[name='buttonGroup']").html("<button type='button' class='stk-button stk-button-st-3' multiple='true' onclick=stk.classifiedSelector.select('"+id+"',this)><span>确定</span></button>");
        var wrap = btng.parent().parent();
        if(wrap.attr('ver') == 'V3'){
            cont.find('input[type=checkbox]').iCheck({
                checkboxClass: 'checkbox'
            }).change(function(){
                $(this).iCheck('update');
            }).on('ifChanged', function(){
        ...
        $("#"+id ).find("input[type='checkbox'][name='classify_all']" ).bind("change",function(){
            var _t = $(this);
            $("#"+id ).find("div[num='"+_t.attr("num")+"']" ).find("input[type='checkbox']" ).prop("checked", _t.prop("checked")).change();
        });
    }
    else
    {

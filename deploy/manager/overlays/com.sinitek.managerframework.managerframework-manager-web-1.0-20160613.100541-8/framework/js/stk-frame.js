		...
		init : function(setting){
			...
			if(typeof opt.menuObj != 'undefined'){
				...
				var hideSearchInput = function(){
                    if(f) return;
					var w = 12;
					if(searchInput.width() < 140){
						w = 140;
					}
                    searchInput.prop('disabled', w==12);
...
//			});
		...
		go : function(url,summaryTitle,setting){
            ...
            if (setting != null && setting.parent != null){
                if (url.indexOf("://") < 0) {
                    if (url.indexOf("/") == 0) {
                        url = setting.parent.location.protocol + "//" + setting.parent.location.host + url;
                    }
                }
            }
		...
		getTab : function(url,summaryTitle,setting){
			...
			var key = encodeURI((opt.keyWithoutParams ? url.split('?')[0] : url) + opt.summaryTitle);
			return CachePageData[key];
		},
